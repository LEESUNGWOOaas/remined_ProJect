					...
					<div class="input-group">
						<div class="input-group-addon">
							<select name="searchType">						
								<option value="title" <c:if test="${userVO.searchType eq 'title'}">selected</c:if>>제목</option>
								<option value="content" <c:if test="${userVO.searchType eq 'content'}">selected</c:if>>내용</option>
							</select>
						</div>
					...
					<div class="table-responsive">
						<table class="table table-bordered table-hover text-center">
							...
							<tbody>
								...
								<tr>
									<td>${rownum }</td>
									<td class="text-left"><a onclick="view(${item.userNo})">${item.title }</a></td>
									<td><c:choose>
